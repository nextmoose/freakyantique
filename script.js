navigator.storage && navigator.storage.persist && navigator.storage.persist().then(granted => granted ? console.log("Persistence") : console.log("No Persistence"));
...
const addData = value => getData().then(alpha => {
    console.log(Array.isArray(alpha)?alpha:[]);
    const x = setData((Array.isArray(alpha)?alpha:[]).concat(value))
    return x;
});
